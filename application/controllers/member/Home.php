...
class Home extends Member_Controller
{
    ...
    public function index()
    {
        ...
        $jumlah_data                            = $this->public_home_model->get_list(array('for_user'=> $this->ion_auth->get_user_id(), 'status' => 0), null, null, array('id','desc'), 'notivikasi')->num_rows();
        
        $config['base_url']                     = site_url('member/home/index/');
        ...
        $this->data['data_parent']              = $this->public_home_model->get_list(array('for_user'=> $this->ion_auth->get_user_id(), 'status' => 0),$config['per_page'], $pagging, array('id','desc'), 'notivikasi')->result();             
        
        $this->load->view('member/home/index', $this->data);
    }
    ...
    public function notiv($id = NULL)
    {
        if($id == NULL){
            ...
            if($cek_data->num_rows() > 0){
                ...
                redirect('member/'.$cek_data->row()->notiv_url, 'refresh');
            }else{
                redirect('member/home', 'refresh');
            }
        }
